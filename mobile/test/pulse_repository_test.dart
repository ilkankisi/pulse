...
void main() {
  ...
  test('like ve unlike doğru HTTP senaryolarını kullanır', () async {
    ...
    expect(requests[2].path, ApiRoutes.postLikes(10));

    expect(requests[3].method, 'GET');
    expect(requests[3].path, ApiRoutes.postLikes(10));
  });
  ...
  test('kullanıcı arama sonucu profile listesine dönüştürülür', () async {
    adapter.onGet(
      ApiRoutes.searchUsers,

      (server) => server.reply(200, <String, dynamic>{
        'items': <Map<String, dynamic>>[_profileJson()],
      }),

      queryParameters: <String, dynamic>{'q': 'ilkan'},
    );

    final profiles = await repository.searchUsers(' ilkan ');

    expect(profiles, hasLength(1));
    expect(profiles.single.username, 'ilkan');
  });

  test('gönderi arama sonucu post listesine dönüştürülür', () async {
    adapter.onGet(
      ApiRoutes.searchPosts,

      (server) => server.reply(200, <String, dynamic>{
        'items': <Map<String, dynamic>>[_postJson()],
      }),

      queryParameters: <String, dynamic>{'q': 'pulse'},
    );

    final posts = await repository.searchPosts(' pulse ');

    expect(posts, hasLength(1));
    expect(posts.single.id, 10);
  });

  test('takipçi listesi sosyal grafik kullanıcılarına dönüştürülür', () async {
    adapter.onGet(
      ApiRoutes.profileFollowers('ilkan'),
    ...
    final followers = await repository.getFollowers('ilkan');

    expect(followers, hasLength(1));
    expect(followers.single.username, 'ada');
  });

  test(
    'takip edilenler listesi sosyal grafik kullanıcılarına dönüştürülür',

    () async {
      adapter.onGet(
        ApiRoutes.profileFollowing('ilkan'),
      ...
      final following = await repository.getFollowing('ilkan');

      expect(following, hasLength(1));
      expect(following.single.username, 'ada');
    },
  );
}
