...
class _ProfilePageState extends ConsumerState<ProfilePage> {
  ...
  Future<void> _openSocialGraph(SocialGraphKind kind) async {
    final username = _profileUsername;
                    ...
                    (
                      ...
                      return ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final isRemoving = removingUsers.contains(
                            user.username,
                          );
                          ...
                          return Column(
                            children: [
                              ListTile(
                                ...
                                trailing:
                                    _isOwnProfile && onRemoveFollower != null
                                    ? TextButton(
                                        key: ValueKey<String>(
                                          'remove-follower-${user.username}',
                                        ),
                    ...
                    Row(
                      children: [
                        ...
                        _ProfileStat(
                          ...
                          onTap: () {
                            unawaited(
                              _openSocialGraph(SocialGraphKind.followers),
                            );
                          },
                        ),
                        const SizedBox(width: 24),
                        _ProfileStat(
                          ...
                          onTap: () {
                            unawaited(
                              _openSocialGraph(SocialGraphKind.following),
                            );
                          },
                        ),
                      ],
...
class _ProfileSettingsPage extends StatelessWidget {
  ...
  Future<void> _exportData(BuildContext context) async {
    final action = exportAccountData;

    if (action == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Veri dışa aktarma şu anda kullanılamıyor.'),
          behavior: SnackBarBehavior.floating,
        ),
      );
      return;
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Veriler dışa aktarılamadı. Tekrar deneyin.'),
          behavior: SnackBarBehavior.floating,
        ),
      );
    }
