...
final createdReply = PulsePost.fromJson(
...
Future<List<PulsePost>> getProfilePosts(
...
Future<PulseProfile> updateMyProfile(
...
ApiRoutes.profileFollow(username),
...
Future<List<PulseSocialGraphUser>> getFollowers(

String username,

) async {

return _getSocialGraph(
...
Future<List<PulseSocialGraphUser>> getFollowing(
...
Future<List<PulseSocialGraphUser>> _getSocialGraph(
...
List<PulseSocialGraphUser> _socialGraphFromJson(
...
if (items is! List) {
  throw const FormatException(
    'Sosyal grafik listesi okunamadı.',
  );
}

return items
    .map(
      (item) => PulseSocialGraphUser.fromJson(
        _asJsonMap(item),
      ),
    )
...
throw const FormatException(
  'Yanıt geçerli bir JSON nesnesi değil.',
);
