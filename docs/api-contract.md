...
Request body taşıyan mutation'larda backend integration testlerinde kullanılan golden JSON canonical wire-format kaynağıdır. Mobil toJson çıktısı ve transport katmanında HTTP'ye gerçekten gönderilen request body birbirleriyle karşılaştırılmakla yetinmez; her biri bağımsız olarak aynı backend golden JSON ile JSON nesnesi semantiğinde eşleşmelidir. Property adı, JSON değer tipi, null/omission semantiği ve enum string değeri aynı olmalıdır; property sırası ve anlamsız whitespace/formatlama farkları eşitliği bozmaz. toJson eşleşip gerçek request body eşleşmiyorsa veya gerçek request body eşleşip toJson eşleşmiyorsa kontrat ihlalidir. Canonical kontratta request body taşımadığı belirtilen mutation'larda mobil istemci JSON/request body üretmemelidir; bu endpoint'lerde golden request-body eşitliği aranmaz.